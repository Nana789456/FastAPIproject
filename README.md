...
ДЗ:
- Переписать функционал работы с видео на UoW.
- Сначала выделить `UnitOfWork`, чтобы он управлял сессией и транзакцией.
- Затем вынести работу с видео в отдельный слой, а endpoint оставить только для HTTP-логики.
- Не менять поведение API: те же ручки, тот же результат, но с более чистой архитектурой.
